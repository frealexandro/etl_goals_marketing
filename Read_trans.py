      ...
      for brand in self.Update:
        if self.Update[brand]['update']:
            ...
            for i in range(goalsNum):
                ...
                df = pd.read_csv(f"C:\\Users\\Santiago\\Documents\\GitHub\\Web_site_goal_final_version\\input\\{file}.csv")
                a =(int(len(df)))
                if a != 0:
                  df = df.set_axis([*df.columns[:-1], 'Value'], axis=1, inplace=False)
                  df_regex = df[df['campaign_name'].str.match('(\w.*\/\/\/)((?:\w+\d+\w+))(\s+)')== True]
                  df_unregex = df[df['campaign_name'].str.match('(\w.*\/\/\/)((?:\w+\d+\w+))(\s+)')== False]
                  fixed_df = df_regex.replace(to_replace=r"\s{3,3}", value=r"\\\\\\", regex=True)
                  df_final_1 =pd.concat([df_final,fixed_df],ignore_index=True)
                  df_final = pd.concat([df_final_1,df_unregex],ignore_index=True)
